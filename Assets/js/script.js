...
function getGeolocation() {
    ...
    var apiKey = "https://api.openweathermap.org/geo/1.0/direct?q=" + search +
        "&limit=1&appid=3d3fa7ea37c6cf9a4c425fcff023bdbd";
...
function getWeather(lat, lon, city) {
    ...
    var apiKey = "https://api.openweathermap.org/data/2.5/onecall?lat=" + lat + "&lon=" + lon +
        "&units=imperial&exclude=minutely,hourly,alerts&appid=3d3fa7ea37c6cf9a4c425fcff023bdbd";
    ...
    if (searchHistory) {
        ...
        for (var i = 0; i < searchHistory.length; i++) {
            ...
            if (mobileHistoryCounter > 3) {
                $("#search-history")
                    .append("<button class='col-12 mt-2 p-1 text-center rounded text-black d-none d-md-block'"
                        + "id='history-btn' value='" + searchHistory[i] + "'>" + searchHistory[i] + "</div");
            }
            else {
                $("#search-history")
                    .append("<button class='col-12 mt-2 p-1 text-center rounded text-black'"
                        + "id='history-btn' value='" + searchHistory[i] + "'>" + searchHistory[i] + "</div");
            }
        }
    }
